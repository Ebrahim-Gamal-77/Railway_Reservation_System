//
// Created by elshehawi on 12/22/24.
//
#pragma once
...
class schedule {
    ...
    static void update();//update a specific thing in the schedule
...
void schedule::update() {
    ...
    int choice = 0;
    cin >> choice;
    cout << "Enter the id of the schedule sir: " << endl;
    int id = 0;
    cin >> id;
    bool flag = false;
    for (auto it : schedules) {
    ...
    if (choice == 1 && flag) {
        ...
        if (option == 1) {
            ...
            cout << "No schedule id with the one you provided" << endl;
